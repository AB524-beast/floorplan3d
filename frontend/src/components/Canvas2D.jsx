...
export default function Canvas2D({ walls, setWalls, setLabels }) {
  ...
  // Canvas context size standard configurations (600x500 as per technical stack specs)
  const width = 600;
  const height = 500;

  // Redraw the canvas state whenever walls array, backgrounds, or manual mouse drags change
  useEffect(() => {
    ...
    // 1. Draw uploaded baseline floorplan image asset
    if (backgroundImage) {
    ...
    // 3. Draw transient preview trace line during an active mouse drag sequence
    if (isDrawing && startPoint && currentMousePos) {
      ctx.strokeStyle = '#3b82f6'; // Interactive Blue
      ctx.lineWidth = 2;
      ctx.setLineDash([4, 4]); // Dashed blueprint trace preview
      ctx.beginPath();
      ctx.moveTo(startPoint.x, startPoint.y);
      ctx.lineTo(currentMousePos.x, currentMousePos.y);
      ctx.stroke();
      ctx.setLineDash([]); // Reset dash line style mapping
    }
  }, [walls, backgroundImage, isDrawing, startPoint, currentMousePos]);

  // Handle local blueprint asset selection upload mapping
  const handleImageUpload = (e) => {
    ...
    canvasRef.current.filePayload = file; // Store raw file handle for the AI API multi-part form data
  };
  ...
  // Capture canvas mouse interface interactions
  const getMousePos = (e) => {
  ...
  const handleMouseUp = (e) => {
    ...
    // Ignore minor accidental clicks
      ...
      <div className="flex items-center justify-between w-full mb-3 gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            title="Clear canvas canvas workspace state"
          >
            <Trash2 size={14} />
          </button>
        </div>
